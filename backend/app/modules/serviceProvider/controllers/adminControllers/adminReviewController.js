...
export const getAllReviews = async (req, res) => {
  try {
    const { page = 1, limit = 10, status, rating, vendorId, serviceId, userId } = req.query;
    ...
    const reviewCount = await SpReview.countDocuments();
    if (reviewCount === 0) {
      const bookingsWithReviews = await SpBooking.find({ rating: { $exists: true, $ne: null } });
      if (bookingsWithReviews.length > 0) {
        ...
        await SpReview.insertMany(reviewsToCreate, { ordered: false }).catch(err => {
    ...
    const reviews = await SpReview.find(query)
    ...
    const total = await SpReview.countDocuments(query);
...
export const updateReviewStatus = async (req, res) => {
  try {
    ...
    const review = await SpReview.findByIdAndUpdate(id, { status }, { new: true });
    if (!review) {
...
export const getReviewStats = async (req, res) => {
  try {
    const stats = await SpReview.aggregate([
      {
        $group: {
          _id: null,
    ...
    const statusStats = await SpReview.aggregate([
