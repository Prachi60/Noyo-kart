...
const getHomeContent = async (req, res) => {
  try {
    ...
    let homeContent = await SpHomeContent.getHomeContent(cityId);
    res.status(200).json({
      success: true,
      homeContent: {
        id: homeContent._id, cityId: homeContent.cityId,
        banners: homeContent.banners || [], promos: homeContent.promos || [],
        curated: homeContent.curated || [], noteworthy: homeContent.noteworthy || [],
        booked: homeContent.booked || [], categorySections: homeContent.categorySections || [],
        ...
        createdAt: homeContent.createdAt, updatedAt: homeContent.updatedAt
      }
    });
  } catch (error) { console.error('Get home content error:', error); res.status(500).json({ success: false, message: 'Failed to fetch home content.' }); }
};
  ...
  try {
    ...
    const sanitizeItems = (items) => {
      ...
      return items.map(item => {
        const newItem = { ...item };
        if (typeof newItem.id === 'string' && (newItem.id.startsWith('hbnr-') || newItem.id.startsWith('hprm-') || newItem.id.startsWith('hcur-') || newItem.id.startsWith('hnot-') || newItem.id.startsWith('hbkd-') || newItem.id.startsWith('hsec-'))) delete newItem.id;
        ...
        if (Array.isArray(newItem.cards)) {
          newItem.cards = newItem.cards.map(card => {
            ...
            if (typeof newCard.id === 'string' && newCard.id.startsWith('hcard-')) delete newCard.id;
            return newCard;
          });
        }
        return newItem;
    ...
    if (req.body.categorySections !== undefined) { homeContent.categorySections = sanitizeItems(req.body.categorySections); homeContent.markModified('categorySections'); }
    ...
    res.status(200).json({ success: true, message: 'Home content updated successfully', homeContent: { id: homeContent._id, cityId: homeContent.cityId, banners: homeContent.banners, promos: homeContent.promos, curated: homeContent.curated, noteworthy: homeContent.noteworthy, booked: homeContent.booked, categorySections: homeContent.categorySections, isActive: homeContent.isActive, isBannersVisible: homeContent.isBannersVisible, isPromosVisible: homeContent.isPromosVisible, isCuratedVisible: homeContent.isCuratedVisible, isNoteworthyVisible: homeContent.isNoteworthyVisible, isBookedVisible: homeContent.isBookedVisible, isCategorySectionsVisible: homeContent.isCategorySectionsVisible, isCategoriesVisible: homeContent.isCategoriesVisible } });
  } catch (error) { console.error('Update home content error:', error); res.status(500).json({ success: false, message: 'Failed to update home content.' }); }
};
