import SpCity from '../models/SpCity.js';
...
const getAllCities = catchAsync(async (req, res) => {
  const cities = await SpCity.find({}).sort({ displayOrder: 1, createdAt: -1 });
...
const getActiveCities = catchAsync(async (req, res) => {
  const cities = await SpCity.find({ isActive: true })
...
const getCity = catchAsync(async (req, res) => {
  const city = await SpCity.findById(req.params.id);
...
const createCity = catchAsync(async (req, res) => {
  ...
  const city = await SpCity.create(req.body);
...
const updateCity = catchAsync(async (req, res) => {
  let city = await SpCity.findById(req.params.id);
  ...
  city = await SpCity.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true
  });
 ...
 * @access  Private (Super Admin)
 */
const toggleCityStatus = catchAsync(async (req, res) => {
  const city = await SpCity.findById(req.params.id);
