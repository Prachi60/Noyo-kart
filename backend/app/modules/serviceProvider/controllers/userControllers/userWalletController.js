...
const getWalletBalance = async (req, res) => {
  try {
    ...
    const user = await SpUser.findById(userId).select('wallet');
...
const getWalletTransactions = async (req, res) => {
  try {
    ...
    // Get transactions
    const transactions = await SpTransaction.find({ userId })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit));

    // Get total count
    const total = await SpTransaction.countDocuments({ userId });
