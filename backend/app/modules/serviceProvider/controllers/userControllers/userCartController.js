...
const getUserCart = async (req, res) => {
  try {
    ...
    let cart = await SpCart.findOne({ userId }).populate('items.serviceId', 'title iconUrl slug').populate('items.categoryId', 'title slug');

    if (!cart) {
      // Create empty cart if doesn't exist
      cart = await SpCart.create({ userId, items: [] });
    }
...
const addToCart = async (req, res) => {
  try {
    ...
    let service = null;
    if (serviceId) {
      service = await SpUserService.findById(serviceId);
      if (!service) {
    ...
    // Get or create cart
    let cart = await SpCart.findOne({ userId });
...
const updateCartItem = async (req, res) => {
  try {
    ...
    const cart = await SpCart.findOne({ userId });
    if (!cart) {
      return res.status(404).json({
        success: false,
