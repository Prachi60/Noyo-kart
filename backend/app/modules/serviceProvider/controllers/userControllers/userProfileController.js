...
const getProfile = async (req, res) => {
  try {
    ...
    const user = await SpUser.findById(userId).select('-password -otp -__v');
  ...
  try {
    ...
    if (email !== undefined) {
      if (email && email.trim()) {
        ...
        const existingUser = await SpUser.findOne({ email: email.toLowerCase(), _id: { $ne: userId } });
        if (existingUser) {
          return res.status(400).json({
            success: false,
    ...
    if (addresses && Array.isArray(addresses)) {
      updateData.addresses = addresses.slice(-1);
    }
...
const getCheckoutData = async (req, res) => {
  try {
    ...
    const [user, cart, settings] = await Promise.all([
      SpUser.findById(userId).select('addresses phone name'),
      SpCart.findOne({ userId }).populate('items.serviceId', 'title iconUrl slug').populate('items.categoryId', 'title slug'),
      SpSettings.findOne({ type: 'global' }).select('visitedCharges serviceGstPercentage partsGstPercentage')
    ]);
