...
const getJobById = async (req, res) => {
  try {
    ...
    const booking = await SpBooking.findOne({ _id: id, workerId })
      .populate('userId', 'name phone email')
  ...
  try {
    ...
    if (status && status !== booking.status) {
      const validTransitions = {
        [SP_BOOKING_STATUS.ASSIGNED]: [SP_BOOKING_STATUS.VISITED, SP_BOOKING_STATUS.IN_PROGRESS],
        [SP_BOOKING_STATUS.CONFIRMED]: [SP_BOOKING_STATUS.ASSIGNED, SP_BOOKING_STATUS.IN_PROGRESS],
        [SP_BOOKING_STATUS.VISITED]: [SP_BOOKING_STATUS.WORK_DONE, SP_BOOKING_STATUS.COMPLETED],
        [SP_BOOKING_STATUS.IN_PROGRESS]: [SP_BOOKING_STATUS.WORK_DONE, SP_BOOKING_STATUS.COMPLETED],
        [SP_BOOKING_STATUS.WORK_DONE]: [SP_BOOKING_STATUS.COMPLETED],
      ...
      if (status === SP_BOOKING_STATUS.IN_PROGRESS && !booking.startedAt) {
        booking.startedAt = new Date();
      }

      if (status === SP_BOOKING_STATUS.VISITED && !booking.startedAt) {
        booking.startedAt = new Date();
      }
      ...
      if (status === SP_BOOKING_STATUS.IN_PROGRESS) {
...
const startJob = async (req, res) => {
  try {
    ...
    if (booking.status !== SP_BOOKING_STATUS.ASSIGNED && booking.status !== SP_BOOKING_STATUS.CONFIRMED && booking.status !== SP_BOOKING_STATUS.ACCEPTED) {
      return res.status(400).json({
        success: false,
    ...
    if (io) {
      io.to(`user_${booking.userId}`).emit('booking_updated', {
        bookingId: booking._id,
        status: SP_BOOKING_STATUS.JOURNEY_STARTED,
        visitOtp: otp
      });
    }
...
const workerReachedLocation = async (req, res) => {
  try {
    ...
    const booking = await SpBooking.findOne({ _id: id, workerId }).select('+visitOtp');
...
const completeJob = async (req, res) => {
  try {
    ...
    if (io) {
      io.to(`user_${booking.userId}`).emit('booking_updated', {
        bookingId: booking._id,
        status: SP_BOOKING_STATUS.WORK_DONE
      });
    }
...
const collectCash = async (req, res) => {
  try {
    ...
    const booking = await SpBooking.findOne({ _id: id, workerId }).select('+paymentOtp');

    if (!booking) {
      return res.status(404).json({ success: false, message: 'Job not found' });
    }

    if (booking.status !== SP_BOOKING_STATUS.WORK_DONE) {
    ...
    if (booking.vendorId) {
      const vendorDoc = await SpVendor.findById(booking.vendorId).select('wallet');
      if (vendorDoc) {
        ...
        await SpVendor.findByIdAndUpdate(booking.vendorId, updateQuery);
