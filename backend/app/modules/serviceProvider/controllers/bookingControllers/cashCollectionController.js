...
import { trackBookingEarning as recordBookingEarning } from '../../services/earningTrackerService.js';
import { createQRCode, getQRCodePayments } from '../../services/razorpayService.js';
 ...
 */
export const initiateOnlineCollection = async (req, res) => {
  try {
    const { id } = req.params;
 ...
 */
export const initiateCashCollection = async (req, res) => {
  try {
    const { id } = req.params;
    const booking = await SpBooking.findById(id);
    ...
    // Force mark modified for nested object
    if (extraItems) {
      booking.markModified('workDoneDetails');
      booking.markModified('extraCharges');
    }
...
export const confirmCashCollection = async (req, res) => {
  try {
    ...
    const updateResult = await SpBooking.updateOne(
    ...
    const vendor = await SpVendor.findById(vendorId).lean();
    let newDues = 0;
    ...
    if (vendor) {
      ...
      await SpVendor.findByIdAndUpdate(vendorId, walletUpdate, { runValidators: false });
 ...
 */
export const customerConfirmPayment = async (req, res) => {
  try {
    const { id } = req.params;
    const booking = await SpBooking.findById(id);
...
export const verifyOnlinePayment = async (req, res) => {
  try {
    ...
    if (booking.status !== SP_BOOKING_STATUS.WORK_DONE && booking.status !== SP_BOOKING_STATUS.COMPLETED) {
    ...
    if (qrRes.success && qrRes.payments && qrRes.payments.length > 0) {
      ...
      if (capturedPayment) {
        ...
        const vendorId = booking.vendorId;
        await SpVendor.findByIdAndUpdate(vendorId, {
 ...
 */
export const confirmManualOnlinePayment = async (req, res) => {
  try {
    const { id } = req.params;
    const { otp } = req.body;
    const userId = req.user._id;
    ...
    // OTP Verification (Mandatory for manual confirmation)
 ...
 */
export const getCashCollectionStatus = async (req, res) => {
  try {
    const { id } = req.params;
    const booking = await SpBooking.findById(id);
    ...
    const statusData = {
      ...
      isPaid: booking.paymentStatus === SP_PAYMENT_STATUS.SUCCESS
    };

    if (booking.razorpayQrId && booking.paymentStatus === SP_PAYMENT_STATUS.PENDING) {
      const qrRes = await getQRCodePayments(booking.razorpayQrId);
      if (qrRes.success && qrRes.payments && qrRes.payments.length > 0) {
