...
const getProfile = async (req, res) => {
  try {
    ...
    const vendor = await SpVendor.findById(vendorId).select('-password -__v');
    if (!vendor) {
    ...
    if (rating === 0) {
      const [ratingData] = await SpBooking.aggregate([
    ...
    const totalJobs = await SpBooking.countDocuments({ vendorId });
    const completedJobs = await SpBooking.countDocuments({ vendorId, status: 'completed' });
