...
import { uploadPaymentScreenshot } from '../../utils/cloudinaryUpload.js';
...
const getWallet = async (req, res) => {
  try {
    ...
    const vendor = await SpVendor.findById(vendorId).select('wallet name businessName');
    ...
    const pendingSettlements = await SpSettlement.countDocuments({
      vendorId,
      status: 'pending'
    });

    // Get total cash collected (sum of all cash_collected transactions)
    const cashCollectedResult = await SpTransaction.aggregate([
      {
        $match: {
          vendorId: vendor._id,
          type: 'cash_collected',
    ...
    const settledResult = await SpTransaction.aggregate([
      {
        $match: {
          vendorId: vendor._id,
          type: 'settlement',
...
const requestSettlement = async (req, res) => {
  try {
    ...
    const existingPending = await SpSettlement.findOne({
      vendorId,
      status: 'pending'
    });
    ...
    const settlement = await SpSettlement.create({
      vendorId,
      amount,
      balanceBefore: currentDues,
      balanceAfter: currentDues - amount,
...
const requestWithdrawal = async (req, res) => {
  try {
    ...
    const pendingWithdrawals = await SpWithdrawal.aggregate([
    ...
    const withdrawal = await SpWithdrawal.create({
      vendorId,
      amount,
      bankDetails,
      adminNotes: notes,
...
const getSettlements = async (req, res) => {
  try {
    ...
    const settlements = await SpSettlement.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit));
...
const getWalletSummary = async (req, res) => {
  try {
    const vendorId = req.user.id;
    const vendor = await SpVendor.findById(vendorId).select('wallet');
    ...
    const todayCollections = await SpTransaction.aggregate([
      {
        $match: {
          vendorId: vendor._id,
          type: 'cash_collected',
    ...
    const weekCollections = await SpTransaction.aggregate([
      {
        $match: {
          vendorId: vendor._id,
          type: 'cash_collected',
...
const payWorker = async (req, res) => {
  try {
    ...
    const booking = await SpBooking.findOne({ _id: bookingId, vendorId });
    ...
    // Record Transaction
    const transaction = new SpTransaction({
      vendorId,
      workerId: worker._id,
...
const getWithdrawals = async (req, res) => {
  try {
    ...
    const withdrawals = await SpWithdrawal.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit));
