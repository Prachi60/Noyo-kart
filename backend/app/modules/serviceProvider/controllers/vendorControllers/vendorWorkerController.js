...
import { SP_WORKER_STATUS, SP_BOOKING_STATUS } from '../../constants.js';
...
const getVendorWorkers = async (req, res) => {
  try {
    ...
    // Get workers
    const workers = await SpWorker.find(query)
      .select('-password')
      .sort({ createdAt: -1 })
      .skip(skip)
...
const addWorker = async (req, res) => {
  try {
    ...
    let worker = await SpWorker.findOne({ phone });
    ...
    if (worker) {
      ...
      if (!worker.vendorId) {
        ...
        worker.status = SP_WORKER_STATUS.ACTIVE;
    ...
    worker = await SpWorker.create({
      ...
      address: address || {},
      status: SP_WORKER_STATUS.ACTIVE
    });
...
const updateWorker = async (req, res) => {
  try {
    ...
    const worker = await SpWorker.findOne({ _id: id, vendorId });
...
const removeWorker = async (req, res) => {
  try {
    ...
    const activeBookings = await SpBooking.countDocuments({
      workerId: id,
      status: { $in: [SP_BOOKING_STATUS.PENDING, SP_BOOKING_STATUS.CONFIRMED, SP_BOOKING_STATUS.IN_PROGRESS] }
    });
    ...
    worker.status = SP_WORKER_STATUS.INACTIVE;
    worker.vendorId = null;
    await worker.save();
