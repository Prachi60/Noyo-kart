    ...
    const loadProfile = async () => {
      try {
        ...
        if (response.success && response.vendor) {
          ...
          localStorage.setItem('spVendorProfile', JSON.stringify(v));
          localStorage.setItem('spVendorData', JSON.stringify(v));
        } else {
    ...
    try {
      ...
      try {
        ...
        if (response.success) {
          ...
          // Update Local Storage
          localStorage.setItem('spVendorProfile', JSON.stringify(updatedProfile));
          localStorage.setItem('spVendorData', JSON.stringify(updatedProfile));

          // Dispatch events
          window.dispatchEvent(new Event('vendorProfileUpdated'));
          window.dispatchEvent(new Event('vendorDataUpdated'));

          navigate('/sp/vendor/profile');
        } else {
    ...
    <div className="min-h-screen pb-20" style={{ background: themeColors.backgroundGradient }}>
      ...
      <main className="px-4 py-6">
        ...
        <div className="mt-8 flex gap-3">
          <button
            onClick={() => navigate('/sp/vendor/profile')}
