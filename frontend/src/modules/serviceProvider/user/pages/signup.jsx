...
import { userAuthService } from '@sp/services/authService';
  ...
  const handleDetailsSubmit = async (e) => {
    ...
    if (verificationToken) {
      try {
        ...
        if (response.success) {
          try {
            const { registerFCMToken } = await import('@sp/services/pushNotificationService');
            await registerFCMToken('user', true);
          } catch (e) { console.error(e); }

          toast.success(
            <div className="flex flex-col">
              <span className="font-bold">Welcome to Appzeto!</span>
    ...
    <div className="min-h-[100dvh] bg-gray-50 flex flex-col justify-start sm:justify-center py-12 sm:px-6 lg:px-8 relative overflow-x-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center mb-8 relative z-10 animate-fade-in">
        ...
        <p className="mt-2 text-sm text-gray-600 animate-stagger-1 animate-fade-in">
          {step === 'details' ? 'Join Appzeto to start booking services' : `We've sent a 6-digit code to ${formData.phoneNumber}`}
        </p>
      </div>
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0 relative z-10">
          ...
          {step === 'details' ? (
            <form onSubmit={handleDetailsSubmit} className="space-y-6">
              {verificationToken && (
                <button
                  type="button"
                  onClick={() => navigate('/sp/user/login')}
        ...
        <p className="mt-8 text-center text-sm text-gray-500 animate-fade-in animate-stagger-5">
          ...
          <Link to="/sp/user/login" className="font-semibold text-[#347989] hover:text-[#D68F35] transition-colors duration-300">
            Sign in
          </Link>
        </p>
      </div>
