...
const workerService = {
  ...
  initiateCashCollection: async (id, totalAmount, extraItems = []) => {
    const response = await api.post(`/bookings/cash/${id}/initiate`, { totalAmount, extraItems });
    return response.data;
  },

  initiateOnlineCollection: async (id, totalAmount, extraItems = []) => {
    const response = await api.post(`/bookings/cash/${id}/initiate-online`, { totalAmount, extraItems });
    return response.data;
  },
  ...
  collectCash: async (id, otp, amount, extraItems = []) => {
    const response = await api.post(`/bookings/cash/${id}/confirm`, { otp, amount, extraItems });
    return response.data;
  },
