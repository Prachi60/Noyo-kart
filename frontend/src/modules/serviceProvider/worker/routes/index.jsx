  ...
  return lazy(() => {
    return Promise.resolve(importFunc()).catch((error) => {
      ...
      if (isChunkError) {
        ...
        const reloadCount = parseInt(sessionStorage.getItem('chunkLoadErrorCount_worker') || '0');
        if (reloadCount < 2) {
          sessionStorage.setItem('chunkLoadErrorCount_worker', (reloadCount + 1).toString());
