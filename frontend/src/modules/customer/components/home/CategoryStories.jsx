...
const stories = [
    ...
    { id: 2, title: 'New Arrival', image: 'https://images.unsplash.com/photo-1615485290382-441e4d019cb5?q=80&w=150&auto=format&fit=crop', color: 'border-[#0284c7]' },
                ...
                <div className="flex gap-4 md:gap-8 overflow-x-auto pb-2 scrollbar-hide snap-x">
                    {stories.map((story) => (
                        <div key={story.id} className="flex flex-col items-center gap-2 flex-shrink-0 cursor-pointer group snap-start">
                            ...
                            <span className="text-[10px] md:text-xs font-black text-slate-700 tracking-tight uppercase group-hover:text-[#0284c7] transition-colors">{story.title}</span>
                        </div>
                    ))}
                </div>
            </div>
